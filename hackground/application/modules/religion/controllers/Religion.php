...
class Religion extends MX_Controller {
   ...
   private $data;
	...
	public function __construct(){
		...
		$this->data['table'] = 'religion';
		$this->data['lang_table'] = 'religion_names';
		...
		$this->religion->configure($model_configuration);
		parent::__construct();
	...
	public function list_record(){
		...
		$offset = 20;
		$this->data['main_title'] = 'Religion Management';
		$this->data['second_title'] = 'All Religion List';
		$this->data['title'] = 'Religion';
		$breadcrumb = array(
			array(
				'name' => 'Religion',
				'path' => '',
		...
		$this->data['list'] = $this->religion->getList($srch, $limit, $offset);
		$this->data['list_total'] = $this->religion->getList($srch, $limit, $offset, FALSE);
	...
	public function load_ajax_page(){
		...
		if($page == 'add'){
			...
			$this->data['detail'] = $this->religion->getDetail($id);
			$this->data['title'] = 'Edit Religion';
		}
	...
	public function add(){
		...
		if(post() && $this->input->is_ajax_request()){
			...
			// $this->form_validation->set_rules('religion_name', 'name', 'required|trim|max_length[100]');
			foreach($lang as $k => $v){
				$this->form_validation->set_rules('lang[religion_name]['.$v.']', "name $v", 'required|trim|max_length[100]');
			}
			$this->form_validation->set_rules('religion_status', 'status', '');
			if($this->form_validation->run()){
				$post = post();
				$insert = $this->religion->addRecord($post);
	...
	public function edit(){
		...
		if(post() && $this->input->is_ajax_request()){
			...
			if($this->form_validation->run()){
				...
				$ID = post('ID');
				unset($post['ID']);
				$update = $this->religion->updateRecord($post, $ID);
				$this->api->cmd('reload');
			}else{
				$errors = validation_errors_array();
				$this->api->set_error($errors);
	...
	public function change_status(){
		if(post() && $this->input->is_ajax_request()){
			...
			if(is_array($ID)){
				$this->db->where_in($this->data['primary_key'], $ID)->update($this->data['table'], array('religion_status' => $sts));
			}else{
				$upd['data'] = array('religion_status' => $sts);
