...
class Religion_model extends CI_Model{
	...
	public function getList($srch=array(), $limit=0, $offset=20, $for_list=TRUE){
		...
		if(!empty($srch['show']) && $srch['show'] == 'trash'){
			$this->db->where('a.religion_status', DELETE_STATUS);	
		}else{
			$this->db->where('a.religion_status <>', DELETE_STATUS);	
		}
		
		if(!empty($srch['term'])){
			$this->db->like('b.religion_name', $srch['term']);
		}
		$this->db->where('b.religion_lang', $admin_default_lang);	
		if($for_list){
	...
	public function addRecord($data=array()){
		$structure = array(
			// 'religion_name' => !empty($data['religion_name']) ? $data['religion_name'] : '',
			'religion_status' => !empty($data['status']) ? $data['status'] : '0',
		);
		$ins['data'] = $structure;
	...
	public function deleteRecord($id=''){
		if($id && is_array($id)){
			return $this->db->where_in($this->primary_key, $id)->update($this->table, array('religion_status' => DELETE_STATUS));
		}else{
			$ins['data'] = array('religion_status' => DELETE_STATUS);
			$ins['table'] = $this->table;
			$ins['where'] = array($this->primary_key => $id);
			return  update($ins);
		}
	...
	public function getDetail($id=''){
		...
		foreach($lang_result as $k => $v){
			$lang_name[$v['religion_lang']] = $v['religion_name'];
		}
		$result['lang'] = array();
		foreach($result as $k => $v){
			$result['lang']['religion_name'] = $lang_name;
		}
		return $result;
	}
