...
class Language_model extends CI_Model{
	...
	public function getList($srch=array(), $limit=0, $offset=20, $for_list=TRUE){
		...
		if(!empty($srch['show']) && $srch['show'] == 'trash'){
			$this->db->where('a.language_status', DELETE_STATUS);	
		}else{
			$this->db->where('a.language_status <>', DELETE_STATUS);	
		}
		
		if(!empty($srch['term'])){
			$this->db->like('b.language_name', $srch['term']);
		}
		$this->db->where('b.language_lang', $admin_default_lang);	
		if($for_list){
	...
	public function getDetail($id=''){
		...
		foreach($lang_result as $k => $v){
			$lang_name[$v['language_lang']] = $v['language_name'];
		}
		$result['lang'] = array();
		foreach($result as $k => $v){
			$result['lang']['language_name'] = $lang_name;
		}
		return $result;
	}
