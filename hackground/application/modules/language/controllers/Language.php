...
class Language extends MX_Controller {
   ...
   private $data;
	...
	public function __construct(){
		...
		$this->data['table'] = 'language';
		$this->data['lang_table'] = 'language_names';
		...
		$this->language->configure($model_configuration);
		parent::__construct();
	...
	public function list_record(){
		...
		$this->data['list'] = $this->language->getList($srch, $limit, $offset);
		$this->data['list_total'] = $this->language->getList($srch, $limit, $offset, FALSE);
	...
	public function load_ajax_page(){
		...
		if($page == 'add'){
			...
			$this->data['detail'] = $this->language->getDetail($id);
	...
	public function add(){
		...
		if(post() && $this->input->is_ajax_request()){
			...
			// $this->form_validation->set_rules('language_name', 'name', 'required|trim|max_length[100]');
			foreach($lang as $k => $v){
				$this->form_validation->set_rules('lang[language_name]['.$v.']', "name $v", 'required|trim|max_length[100]');
			}
			$this->form_validation->set_rules('language_status', 'status', '');
			if($this->form_validation->run()){
				$post = post();
				$insert = $this->language->addRecord($post);
	...
	public function edit(){
		...
		if(post() && $this->input->is_ajax_request()){
			...
			if($this->form_validation->run()){
				...
				$ID = post('ID');
				unset($post['ID']);
				$update = $this->language->updateRecord($post, $ID);
				$this->api->cmd('reload');
			}else{
				$errors = validation_errors_array();
				$this->api->set_error($errors);
	...
	public function change_status(){
		if(post() && $this->input->is_ajax_request()){
			...
			if(is_array($ID)){
				$this->db->where_in($this->data['primary_key'], $ID)->update($this->data['table'], array('language_status' => $sts));
			}else{
				$upd['data'] = array('language_status' => $sts);
