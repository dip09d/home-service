...
class Member_model extends CI_Model{
	...
	public function getList($srch=array(), $limit=0, $offset=20, $for_list=TRUE){
		...
		if(!empty($srch['term'])){
			...
			$this->db->or_like('u.member_phone', $srch['term']);
			$this->db->group_end();
		}
	...
	public function updateRecord($data=array(), $id=''){
		$structure = array(
			...
			'member_phone' => !empty($data['member_phone']) ? $data['member_phone'] : '',
		);
		$ins['data'] = $structure;
	...
	private function _getMemberAddress($member_id=''){
		$member_address = $this->db->where('member_id', $member_id)->where('address_status',1)->get('member_address')->result_array();
