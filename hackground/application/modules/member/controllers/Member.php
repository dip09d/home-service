...
class Member extends MX_Controller {
   ...
   private $data;
	...
	public function list_record(){
		...
		$this->data['main_title'] = 'Customer Management';

		$this->data['second_title'] = 'All Customer List';
	...
	public function edit(){
		...
		if(post() && $this->input->is_ajax_request()){
			...
			$this->form_validation->set_rules('member_email', 'email', 'trim|max_length[100]|valid_email');
			...
			if($this->form_validation->run()){
				...
				$this->db->where('access_user_id',$profile_connection->access_user_id)->update('access_panel',array('login_status'=>$login_status,'access_user_email'=>$post['member_phone']));
				/* if($this->input->post('new_pass')){
	...
	public function view_edit($module='', $member_id=''){
		...
		$this->data['main_title'] = 'Customer Detail ';
		$this->data['second_title'] =  $this->data['detail']['member_name'] ;
		
		
		$this->data['module'] = $module;
		$this->data['member_id'] = $member_id;
		$this->data['organization_id'] = isset($this->data['detail']['organization_id']) ? $this->data['detail']['organization_id'] : '';
		if($module == 'basic_info'){
			$this->_member_basic_info($member_id);
		}else if($module == 'address'){
			$this->_member_address($member_id);
		}else{
			show_404();
			return;
		}
	...
	private function _member_address($member_id=''){
	...
	public function edit_member_info(){
		if(post() && $this->input->is_ajax_request()){
			...
			}else if($page == 'member-address'){
				$this->load->library('form_validation');
				
				$this->form_validation->set_rules('name', 'name', 'required');
				$this->form_validation->set_rules('member_address_1', 'address line 1', 'required');
				$this->form_validation->set_rules('member_landmark', 'landmark', 'required');

				$this->form_validation->set_rules('member_city', 'city', 'required');
				$this->form_validation->set_rules('member_state', 'state', 'required');
				$this->form_validation->set_rules('member_pincode', 'pincode', 'required');
				
				if($this->form_validation->run()){
					
					$member_id = post('ID');
					$dataid = post('member_address_id');
					
					$memberDatacount=getData(array(
						'select'=>'m_e.member_address_id',
						'table'=>'member_address as m_e',
						'where'=>array('m_e.member_id'=>$member_id,'m_e.member_address_id'=>$dataid),
						'single_row'=>true,
					));
					$dataparse=[
						'name'=>post('name'),
						'member_state'=>post('member_state'),
						'member_city'=>post('member_city'),
						'member_landmark'=>post('member_landmark'),
						'member_address_1'=>post('member_address_1'),
						'member_address_2'=>post('member_address_2'),
						'member_pincode'=>post('member_pincode'),
						'address_status'=>1
					];

					if($memberDatacount){
						$up=updateTable('member_address',$dataparse,array('member_id'=>$member_id,'member_address_id'=>$memberDatacount->member_address_id));
					}else{
						$dataparse['member_id']=$member_id;
						$up=insert_record('member_address',$dataparse,TRUE);
					}
	...
	public function ajax_modal(){
		...
		if($page == 'member-language'){
			...
			$this->data['member_address_id'] = get('member_address_id');
			$this->data['ID'] = get('ID');
	...
	public function delete_data(){
		...
		if($formtype=='language'){
			...
			if($memberemployment){
				$up=updateTable('member_address',array('address_status'=>0),array('member_address_id'=>$memberemployment->member_address_id));
			}
