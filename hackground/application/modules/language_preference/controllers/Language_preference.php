...
class Language_preference extends MX_Controller {
   ...
   private $data;
	...
	public function __construct(){
		...
		$this->load->model('language_preference_model', 'language_preference');
		$this->data['table'] = 'language_preference';
		$this->data['lang_table'] = 'language_preference_names';
		...
		$this->language_preference->configure($model_configuration);
		parent::__construct();
	...
	public function list_record(){
		...
		$offset = 20;
		$this->data['main_title'] = 'Language Preference Management';
		$this->data['second_title'] = 'All Language Preference List';
		$this->data['title'] = 'Language Preference';
		$breadcrumb = array(
			array(
		...
		$this->data['list'] = $this->language_preference->getList($srch, $limit, $offset);
		$this->data['list_total'] = $this->language_preference->getList($srch, $limit, $offset, FALSE);
	...
	public function load_ajax_page(){
		...
		$this->data['page'] = $page;
		if($page == 'add'){
			$this->data['title'] = 'Add Language Preference';
			...
			$this->data['detail'] = $this->language_preference->getDetail($id);
			$this->data['title'] = 'Edit Language Preference';
		}
	...
	public function add(){
		...
		if(post() && $this->input->is_ajax_request()){
			...
			// $this->form_validation->set_rules('language_preference_name', 'name', 'required|trim|max_length[100]');
			// $this->form_validation->set_rules('language_preference_info', 'info', 'required|trim');
			foreach($lang as $k => $v){
				$this->form_validation->set_rules('lang[language_preference_name]['.$v.']', "name $v", 'required|trim');
				$this->form_validation->set_rules('lang[language_preference_info]['.$v.']', "info $v", 'required|trim');
			}
			$this->form_validation->set_rules('language_preference_status', 'status', '');
			if($this->form_validation->run()){
				$post = post();
				$insert = $this->language_preference->addRecord($post);
	...
	public function edit(){
		...
		if(post() && $this->input->is_ajax_request()){
			...
			if($this->form_validation->run()){
				...
				$ID = post('ID');
				unset($post['ID']);
				$update = $this->language_preference->updateRecord($post, $ID);
				$this->api->cmd('reload');
			}else{
				$errors = validation_errors_array();
				$this->api->set_error($errors);
	...
	public function change_status(){
		if(post() && $this->input->is_ajax_request()){
			...
			if(is_array($ID)){
				$this->db->where_in($this->data['primary_key'], $ID)->update($this->data['table'], array('language_preference_status' => $sts));
			}else{
				$upd['data'] = array('language_preference_status' => $sts);
	...
	public function delete_record($id=''){
		...
		if($id){
			$this->language_preference->deleteRecord($id);
			$cmd = get('cmd');
			if($cmd && $cmd == 'remove'){
				if($id && is_array($id)){
