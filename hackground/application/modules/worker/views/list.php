      ...
      <div class="card">
        <div class="card-header">
          ...
          <div class="card-tools">
			<a href="<?php echo base_url('worker/export_csv?export=1&'. http_build_query(array_merge($_GET, ['export' => 1])))?>"
			class="btn btn-success btn-sm mr-2">
				<i class="icon-feather-download"></i>
				Export CSV
			</a>
		  	<a class="btn btn-success btn-sm" target="_blank" href="<?php echo SITE_URL.'sign-up/'; ?>">Add New</a>
          </div>
        </div>
		...
		<div class="card-body table-responsive p-0" id="main_table">
              <table class="table table-hover">
				...
				$logo = getWorkerLogo($v[$primary_key]);
				
				
				$user_tag = '';
				if($v['worker_gender'] == 'F'){
					$user_tag = '<span class="badge badge-dark">Female</span>';
				}elseif($v['worker_gender'] == 'M'){
					$user_tag = '<span class="badge badge-info">Male</span>';
				}
				...
				if($v['worker_payment_status']=='PAID'){
					$payment_status = '<span class="badge badge-success">Paid</span>';
				}else{
					$payment_status = '<span class="badge badge-secondary">Unpaid</span>';
				}
				?>
				<tr>
                  ...
                  <td>
					<div class="d-flex">
						  <img src="<?php echo $logo;?>" class="rounded-circle mr-2" alt="User Image" height="32" width="32" /> 
					  <div>
					  <?php echo $v['worker_name']; ?>
					 	<p class="small">Age: <span style="display:inline-block; min-width: 18px;"><?php echo $v['age']; ?></span> <span><?php echo $user_tag; ?> </span> <?php echo $payment_status;?></p>
						</div>
					</div>
					</td>
				  <td><?php echo $v['worker_phone']; ?></td>
				    <?php
						$names = array_map('trim', explode(',', $v['sub_cat_names']));
						$visible = array_slice($names, 0, 2);
						$hiddenCount = count($names) - 2;
						$allNames = implode(', ', $names);
					?>
				    <td>
						<span class="subcat-wrapper" data-toggle="tooltip" title="<?php echo htmlspecialchars($allNames); ?>">
							<?php echo implode(', ', $visible); ?>

							<?php if ($hiddenCount > 0): ?>
								<span class="more-subcat">
									+<?php echo $hiddenCount; ?>
								</span>
							<?php endif; ?>
						</span>
					</td>
                  ...
                  <!-- <td><?php //echo date('d M, Y h:i A', strtotime($v['worker_register_date'])); ?></td> -->
                  <td><?php echo date("d M Y h:i A", strtotime($v['worker_register_date'] . ' UTC')); ?></td>
                  ...
                  <td class="text-right" style="padding-right:15px;">
                      ...
                      <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                        ...
                        <a class="dropdown-item" href="<?php echo base_url('worker/view_edit/basic_info/'.$v[$primary_key]); ?>">Edit Full Details</a>
                        <a class="dropdown-item" href="<?php echo JS_VOID;?>" onclick="edit('<?php echo $v[$primary_key]; ?>')">Edit Basic Info</a>                        
                        
                        <a class="dropdown-item" href="<?php echo base_url('wallet/list_record/?worker_id='.$v[$primary_key]); ?>" target="_blank">View Wallet</a>                       
                      </div>
                    </div>
					...
					<?php /*
					&nbsp;
					<a href="<?php echo base_url('proposal/list_record?worker_id='.$v[$primary_key]); ?>" data-toggle="tooltip" title="View Projects"><i class="fa fa-list-alt <?php echo ICON_SIZE;?>"></i></a>
					&nbsp;
					<a href="<?php echo base_url('worker/view_edit/basic_info/'.$v[$primary_key]); ?>" data-toggle="tooltip" title="Full Detail"><i class="fa fa-user-secret <?php echo ICON_SIZE;?> red"></i></a>*/?>
				  </td>
