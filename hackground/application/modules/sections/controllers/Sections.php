...
class Sections extends MX_Controller {
   ...
   private $data;
	...
	public function __construct(){
		...
		$this->data['table'] = 'section_boxes';
		$this->data['lang_table'] = 'section_boxes_names';
		$this->data['primary_key'] = 'box_id';
		...
		$this->section->configure($model_configuration);
	...
	public function list_record(){
		...
		$this->data['list'] = $this->section->getList($srch, $limit, $offset);
		$this->data['list_total'] = $this->section->getList($srch, $limit, $offset, FALSE);
	...
	public function add(){
		...
		if(post() && $this->input->is_ajax_request()){
			...
			if($this->form_validation->run()){
				$post = post();
				$insert = $this->section->addRecord($post);
	...
	public function edit(){
		...
		if(post() && $this->input->is_ajax_request()){
			...
			if($this->form_validation->run()){
				...
				$ID = post('ID');
				unset($post['ID']);
				$update = $this->section->updateRecord($post, $ID);
				$this->api->cmd('reload');
			}else{
				$errors = validation_errors_array();
				$this->api->set_error($errors);
