...
class Sub_category_model extends CI_Model{
	...
	public function getList($srch=array(), $limit=0, $offset=20, $for_list=TRUE){
		...
		if(!empty($srch['show']) && $srch['show'] == 'trash'){
			$this->db->where('a.category_subchild_status', DELETE_STATUS);	
		}else{
			$this->db->where('a.category_subchild_status <>', DELETE_STATUS);	
		}
		...
		if(!empty($srch['term'])){
			$this->db->group_start();
			$this->db->like('b.category_subchild_name', $srch['term']);
			$this->db->or_like('a.category_subchild_key', $srch['term']);
			$this->db->group_end();
	...
	public function addRecord($data=array()){
		$structure = array(
			'category_subchild_key' => !empty($data['category_subchild_key']) ? $data['category_subchild_key'] : '',
			/* 'category_subchild_icon' => !empty($data['category_subchild_icon']) ? $data['category_subchild_icon'] : '', */
			'category_id' => !empty($data['category_id']) ? $data['category_id'] : '',
			'price' => !empty($data['price']) ? $data['price'] : '0',
			'next_hour_price' => !empty($data['next_hour_price']) ? $data['next_hour_price'] : '0',
			'late_night_price' => !empty($data['late_night_price']) ? $data['late_night_price'] : '0',
			'category_subchild_status' => !empty($data['status']) ? $data['status'] : '0',
			'category_subchild_order' => !empty($data['category_subchild_order']) ? $data['category_subchild_order'] : '0',
			/* 'is_popular' => !empty($data['is_popular']) ? $data['is_popular'] : '0',
			'is_module' => !empty($data['is_module']) ? $data['is_module'] : '', */
			'category_subchild_thumb' => !empty($data['category_subchild_thumb']) ? $data['category_subchild_thumb'] : '',
		);
		$ins['data'] = $structure;
	...
	public function deleteRecord($id=''){
		if($id && is_array($id)){
			return $this->db->where_in($this->primary_key, $id)->update($this->table, array('category_subchild_status' => DELETE_STATUS));
		}else{
			$ins['data'] = array('category_subchild_status' => DELETE_STATUS);
			$ins['table'] = $this->table;
			$ins['where'] = array($this->primary_key => $id);
			return  update($ins);
		}
	...
	public function getDetail($id=''){
		...
		foreach($lang_result as $k => $v){
			$lang_name[$v['category_subchild_lang']] = $v['category_subchild_name'];
			$lang_info[$v['category_subchild_lang']] = $v['description'];
		}
		$result['lang'] = array();
		foreach($result as $k => $v){
			$result['lang']['category_subchild_name'] = $lang_name;
			$result['lang']['description'] = $lang_info;
		}
		return $result;
	}
	
	public function get_category_name($category_subchild_id=''){
		$admin_default_lang = admin_default_lang();
		$this->db->select('*')
		...
		$this->db->where('b.category_subchild_lang', $admin_default_lang);	
		$this->db->where('a.category_subchild_id', $category_subchild_id);	
		$result = $this->db->get()->row_array();
		return !empty($result['category_subchild_name']) ? $result['category_subchild_name'] : '';
	}
	
	public function hasNoChild($cat_id=''){
		$count = (bool) $this->db->where('category_subchild_id', $cat_id)->count_all_results('category_subchild_level_3');
		if($count > 0){
			return FALSE;
		}
