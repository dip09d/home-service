        ...
        <table class="table table-hover">
          <tbody>
            ...
            <?php if(count($list) > 0){foreach($list as $k => $v){ 
				...
				}else if($v['category_subchild_status'] == INACTIVE_STATUS){
...
function manage_component(id){
	var url = '<?php echo base_url('component/load_ajax_page?page=add_category_component&category_level=2&category_subchild_id=');?>'+id;
	load_ajax_modal(url);
}


function add_sub_category(cat_id){
	var url = '<?php echo base_url('sub_category_level_three/load_ajax_page?page='.$add_command);?>&category_subchild_id='+cat_id;
	load_ajax_modal(url);
}

function add(){
	var url = '<?php echo base_url($curr_controller.'load_ajax_page?page='.$add_command.'&category_id='.$category_id);?>';
	load_ajax_modal(url);
}
