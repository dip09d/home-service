...
class Sub_category extends MX_Controller {
   ...
   private $data;
	...
	public function __construct(){
		...
		$this->data['table'] = 'category_subchild';
		$this->data['lang_table'] = 'category_subchild_names';
		...
		$this->sub_category->configure($model_configuration);
	...
	public function list_record($category_id=''){
		...
		$this->data['category_name'] = $category_name = $this->category_model->get_category_name($category_id);
		$srch = get();
		...
		$this->data['list'] = $this->sub_category->getList($srch, $limit, $offset);
		$this->data['list_total'] = $this->sub_category->getList($srch, $limit, $offset, FALSE);
		...
		$this->data['add_btn'] = 'Add Speciality';
	...
	public function load_ajax_page(){
		...
		if($page == 'add'){
			$this->data['category_id'] = get('category_id');
			if($this->data['category_id']){
				$this->data['category_name'] =$this->category_model->get_category_name($this->data['category_id']);
			}else{
				$this->data['category_name'] ='';
			}
			
			$this->data['category'] = $this->category_model->get_all_category();
			$this->data['title'] = 'Add Speciality';
			...
			$this->data['detail'] = $this->sub_category->getDetail($id);
			
			$this->data['title'] = 'Edit Speciality';
		}
	...
	public function add(){
		...
		if(post() && $this->input->is_ajax_request()){
			...
			$this->form_validation->set_rules('next_hour_price', 'next hour price', 'required');
			$this->form_validation->set_rules('status', 'status', '');
			$this->form_validation->set_rules('category_subchild_key', 'category key', 'required|regex_match[/^[a-z\-A-Z]+$/]|is_unique[category_subchild.category_subchild_key]');
			if($this->form_validation->run()){
				$post = post();
				$insert = $this->sub_category->addRecord($post);
	...
	public function edit(){
		...
		if(post() && $this->input->is_ajax_request()){
			...
			if($this->form_validation->run()){
				...
				$ID = post('ID');
				unset($post['ID']);
				$update = $this->sub_category->updateRecord($post, $ID);
				$this->api->cmd('reload');
			}else{
				$errors = validation_errors_array();
				$this->api->set_error($errors);
	...
	public function delete_record($id=''){
		...
		if($id){
			$this->sub_category->deleteRecord($id);
			$cmd = get('cmd');
			if($cmd && $cmd == 'remove'){
				if($id && is_array($id)){
