...
if($is_invoice_paid == 1){
	...
	$recipient_location_address=array($recipient_information['R_flat'],$recipient_information['R_street'],$recipient_information['R_addr']);
	...
	$receiver_information_address[]='<span>'.$recipient_information['R_state'].'</span>';
...
if(count($InvoiceDetails->invoice_row) > 0){
	foreach($InvoiceDetails->invoice_row as $k => $v){
		$amount =$v->invoice_row_amount*$v->invoice_row_unit_price;
		$total_amount_array[] = $amount ;
		/* $tax_percent=0;
		$saving=$v->product_discount*$v->invoice_row_amount;
		if($v->tax_info){
			$tax_percent=$v->tax_info->igst+$v->tax_info->cgst+$v->tax_info->sgst+$v->tax_info->cess;
			$row_name=$tax_percent;
			$taxamount=$v->product_total-($v->igst+$v->cgst+$v->sgst+$v->cess);
			$tax_break_up_row[$row_name]['item'][]=array(
				'igst'=>$v->tax_info->igst,
				'cgst'=>$v->tax_info->cgst,
				'sgst'=>$v->tax_info->sgst,
				'cess'=>$v->tax_info->cess,
				'igst_amount'=>$v->igst,
				'cgst_amount'=>$v->cgst,
				'sgst_amount'=>$v->sgst,
				'cess_amount'=>$v->cess,
			);
			$tax_break_up_row[$row_name]['amount'][]=$taxamount;
			$tax_break_up_row[$row_name]['gst']=$v->tax_info->igst+$v->tax_info->cgst+$v->tax_info->sgst;
			$tax_break_up_row[$row_name]['cess']=$v->tax_info->cess;
			$tax_break_up_row[$row_name]['igst_amount'][]=$v->igst;
			$tax_break_up_row[$row_name]['cgst_amount'][]=$v->cgst;
			$tax_break_up_row[$row_name]['sgst_amount'][]=$v->sgst;
			$tax_break_up_row[$row_name]['cess_amount'][]=$v->cess;
		} */
		$inv_row  .= '<tr>
		<td class="row_table_item" style="width:60%">'.$v->invoice_row_text.'</td>
		<td class="row_table_item center" style="width:10%">'.$v->invoice_row_amount.'</td>
		<td class="row_table_item center" style="width:15%">'.$v->invoice_row_unit_price.'</td>
		<td class="row_table_item center " style="width:15%">'.$amount.'</td>
		</tr>';
...
$net_total = round($InvoiceDetails->total,2);
$total_gst = 0;

$inv_row_summery='';
/* $inv_row_summery  .= '<tr>
		<td colspan="2" class="row_table_item summery-item" style="width:50%;" align="center">
		Summary
		</td>
		<td class="row_table_item center summery-item" style="width:10%">'.$total_qty.'</td>
		<td class="row_table_item center summery-item" style="width:15%">'.$total_mrp.'</td>
		<td class="row_table_item center summery-item" style="width:10%">'.$total_saving.'</td>
		<td class="row_table_item center summery-item " style="width:15%">'.$total_amount.'</td>
		</tr>'; */

$tax_bearkup_table_row='';
/* $total_taxable_amount=$total_igst_amount=$total_cgst_amount=$total_sgst_amount=$total_cess_amount=0;
if($tax_break_up_row){
	
	foreach($tax_break_up_row as $k=>$row){
		$gst_percent=$row['gst'];
		$cess_percent=$row['cess'];
		$taxable_amount=array_sum($row['amount']);
		$igst_amount=array_sum($row['igst_amount']);
		$cgst_amount=array_sum($row['cgst_amount']);
		$sgst_amount=array_sum($row['sgst_amount']);
		$cess_amount=array_sum($row['cess_amount']);
		$tax_bearkup_table_row  .= '<tr>
		<td class="row_table_item center summery-item" style="width:30%">'.$gst_percent.' ('.$cess_percent.')</td>
		<td class="row_table_item center summery-item" style="width:20%">'.round($taxable_amount,2).'</td>
		<td class="row_table_item center summery-item" style="width:10%">'.$igst_amount.'</td>
		<td class="row_table_item center summery-item" style="width:15%">'.$cgst_amount.'</td>
		<td class="row_table_item center summery-item " style="width:15%">'.$sgst_amount.'</td>
		<td class="row_table_item center summery-item " style="width:10%">'.$cess_amount.'</td>
		</tr>';
...
$tax_bearkup_table_row  .= '<tr>
		<td class="row_table_item center summery-item" style="width:30%">Total</td>
		<td class="row_table_item center summery-item" style="width:20%">'.round($total_taxable_amount,2).'</td>
		<td class="row_table_item center summery-item" style="width:10%">'.$total_igst_amount.'</td>
		<td class="row_table_item center summery-item" style="width:15%">'.$total_cgst_amount.'</td>
		<td class="row_table_item center summery-item " style="width:15%">'.$total_sgst_amount.'</td>
		<td class="row_table_item center summery-item " style="width:10%">'.$total_cess_amount.'</td>
	</tr>';
$tax_bearkup_table='
<tr>
	<th colspan="1"></th>
	<th colspan="5" >
	<table cellpadding="6"  style="padding-bottom-10px;" cellspacing=0>
		<thead>
		<tr>
		<th class="row_table_head center" style="width:30%;font-size:8px">GST (CESS) %</th>
		<th class="row_table_head center" style="width:20%;font-size:8px">Taxable Amount</th>
		<th class="row_table_head center" style="width:10%;font-size:8px">IGST</th>
		<th class="row_table_head center" style="width:15%;font-size:8px">SGST</th>
		<th class="row_table_head center" style="width:15%;font-size:8px">CGST</th>
		<th class="row_table_head center" style="width:10%;font-size:8px">CESS</th>
		</tr>
		</thead>
			<tbody>
				'.$tax_bearkup_table_row.'
...
$invoice_times='';
/* $invoice_date=getFieldData('reg_date','order_status','','',array('order_id'=>$orderDetails->order_id,'order_status'=>5));
if($invoice_date){
	$invoice_times='<tr><td class="referance_css">Invoice Time :</td><td class="right">'.date('M dS ,Y h:i:s A', strtotime($invoice_date)).'</td></tr>';
} */
...
<th class="row_table_head center" style="width:10%">QTY</th>
<th class="row_table_head center" style="width:15%">Price ($c)</th>
...
<th style="padding: 8px;border-top: 1px solid #ccc;text-align:center;" class="row_table_item">{$total_amount}</th>
</tr>
...
<!--<tr>
<th colspan="{$extraRow_sub}"></th>
<th colspan="2" style="padding: 8px;border-left: 1px solid #fff;border-right: 1px solid #fff;color:#fff;border-top: 1px solid #ccc;background-color:#007fff;font-weight:bold;font-size:11px;">Total GST ($c)</th>
<th style="padding: 8px;border-top: 1px solid #ccc;text-align:center;background-color:#007fff;color:#fff;font-weight:bold;font-size:11px;">{$total_gst}</th>
</tr>-->
